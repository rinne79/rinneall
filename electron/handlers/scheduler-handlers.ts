...
const SCHEDULER_METADATA_PATH = path.join(os.homedir(), '.claude-command-center', 'scheduler-metadata.json');
...
function resolveLogPath(taskId: string): string {
  ...
  if (fs.existsSync(commandCenterPlistPath)) {
    try {
      const plistContent = fs.readFileSync(commandCenterPlistPath, 'utf-8');
      const stdOutMatch = plistContent.match(/<key>StandardOutPath<\/key>\s*<string>([^<]+)<\/string>/);
      if (stdOutMatch) return stdOutMatch[1];
    } catch { /* use default */ }
  }
...
export function registerSchedulerHandlers(deps: SchedulerDeps): void {
        ...
        if (fs.existsSync(launchAgentsDir)) {
          try {
            ...
            for (const file of files) {
              if (!file.startsWith('com.claude.schedule.') && !file.startsWith('com.claude-command-center.scheduler.')) continue;
              ...
              if (file.startsWith('com.claude.schedule.')) {
                ...
                taskId = file.replace('com.claude-command-center.scheduler.', '').replace('.plist', '');
              }
  ...
  ipcMain.handle('scheduler:deleteTask', async (_event, taskId: string) => {
    try {
      ...
      if (os.platform() === 'darwin') {
        const labels = [
          `com.claude-command-center.scheduler.${taskId}`,
