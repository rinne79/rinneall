 ...
 */
export function ensureCommandCenterClaudeMd(): void {
  try {
    ensureDataDir();
...
/**
 * Migrate data from ~/.claude-manager to ~/.claude-command-center on first launch after rebrand.
...
export function migrateFromClaudeManager() {
  ...
  console.log('Migrating data from ~/.claude-manager to ~/.claude-command-center...');
  ...
  for (const item of items) {
    ...
    if (fs.existsSync(dest)) {
      console.log(`  Skipping ${item} (already exists in ~/.claude-command-center)`);
      continue;
    }
