...
export default function AgentKnowledgeGraph() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // - ~/.claude-command-center/CLAUDE.md (global Rinne Command Center config)
      // - {projectPath}/CLAUDE.md and {projectPath}/.claude/CLAUDE.md per agent
      const cmds = [
        `[ -f "$HOME/.claude/CLAUDE.md" ] && echo "$HOME/.claude/CLAUDE.md"`,
        `[ -f "$HOME/.claude-command-center/CLAUDE.md" ] && echo "$HOME/.claude-command-center/CLAUDE.md"`,
      ...
      for (const fp of foundPaths) {
        // Global: ~/.claude/ or ~/.claude-command-center/ files
        const isGlobal = (fp.includes('/.claude/') && !fp.includes('/.claude/projects/'))
          || fp.includes('/.claude-command-center/');
        if (isGlobal) {
