...
vi.mock('../../electron/constants', () => ({
  GITHUB_REPO: 'Samin12/claude-command-center-beta',
}));
  ...
  describe('checkForUpdates', () => {
    ...
    it('falls back to GitHub API when autoUpdater throws', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        'https://api.github.com/repos/Samin12/claude-command-center-beta/releases/latest',
    ...
    it('sends update-not-available IPC via fallback when on latest', async () => {
      ...
      mockFetch.mockResolvedValue({
        ...
        json: async () => ({
          tag_name: 'v1.2.1',
          html_url: 'https://github.com/Samin12/claude-command-center-beta/releases/tag/v1.2.1',
          body: '',
          assets: [],
        }),
      });
